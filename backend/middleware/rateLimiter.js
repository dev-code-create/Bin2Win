...
export const createRateLimiter = (options = {}) => {
  ...
  return (req, res, next) => {
    ...
    if (legacyHeaders) {
      ...
      res.set('X-RateLimit-Reset', Math.ceil((Date.now() + timeToReset) / 1000));
    }
...
export const otpRateLimiter = createRateLimiter({
  ...
  message: 'Too many OTP requests for this phone number, please try again after 5 minutes.',
  keyGenerator: (req) => `otp:${req.body.phoneNumber || req.ip}`,
  onLimitReached: (req, res) => {
    console.log(`OTP rate limit exceeded: ${req.body.phoneNumber || req.ip} at ${new Date().toISOString()}`);
  }
});
...
export const submissionRateLimiter = createRateLimiter({
  ...
  message: 'Too many waste submissions, please wait a moment before submitting again.',
...
export const redemptionRateLimiter = createRateLimiter({
  ...
  message: 'Too many redemption attempts, please wait a moment before trying again.',
...
export const dynamicRateLimiter = (req, res, next) => {
  ...
  if (req.user) {
    ...
    if (req.user.stats?.rank === 'Diamond' || req.user.stats?.rank === 'Platinum') {
      maxRequests = 300;
    }
  } else if (req.admin) {
