  ...
  async getAllRewards(req, res) {
    try {
      ...
      if (req.user && req.user.userId) {
        const user = await User.findById(req.user.userId).select('greenCredits');
      ...
      const rewardsWithAvailability = rewards.map(reward => ({
        ...
        canAfford: userCredits !== null ? userCredits >= reward.pointsCost : null,
  ...
  async getRewardById(req, res) {
    try {
      ...
      const reward = await Reward.findById(rewardId)
        .populate('sponsor', 'name contactInfo website');
  ...
  async createReward(req, res) {
    try {
      ...
      if (!name || !description || !pointsCost || !category || !stock) {
        return res.status(400).json({
          success: false,
          message: 'Name, description, points cost, category, and stock are required'
        });
      }
